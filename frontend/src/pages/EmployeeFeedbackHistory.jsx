import { ArrowBack, Insights, Send, Star, Timeline } from '@mui/icons-material';
import { Avatar, Box, Button, Card, CircularProgress, Container, Grid, IconButton, TextField, Tooltip, Typography } from '@mui/material';
            ...
            <Box sx={{ mt: 2 }}>
                ...
                {loadingComments ? (
                    ...
                    comments.map(comment => (
                        <Box key={comment.id} sx={{ mb: 1, pl: 1, borderLeft: '3px solid #eee', display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Avatar sx={{ width: 24, height: 24, fontSize: 14 }}>{comment.user_name?.charAt(0) || 'U'}</Avatar>
                            <Box>
                                <Typography variant="caption" sx={{ fontWeight: 600 }}>{comment.user_name || 'User'}:</Typography>
                ...
                <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                    ...
                    <Button
                        ...
                        sx={{ borderRadius: '12px', minWidth: 'auto', px: 3 }}
                    >
                        <Send />
                    </Button>
                </Box>
...
function EmployeeFeedbackHistory() {
    const { user } = useAuth();
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            setLoading(true);
            try {
                const res = await api.getEmployeeFeedback(user.id);
...
export default EmployeeFeedbackHistory; 